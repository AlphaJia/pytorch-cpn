...
class MscocoMulti(data.Dataset):
    def __init__(self, cfg, train=True):
        ...
        with open(cfg.gt_path) as anno_file:
            self.anno_temp = json.load(anno_file)
            self.anno = self.bbox_refine(self.anno_temp)

    @staticmethod
    def bbox_refine(temp_json):
        result = []
        for i in range(len(temp_json)):
            a = temp_json[i]
            gt_bbox = a['bbox']
            if gt_bbox[2] - gt_bbox[0] > 10 and gt_bbox[3] - gt_bbox[1] > 10:
                result.append(temp_json[i])
    ...
    def __getitem__(self, index):
        ...
        file_name = str(image_name).zfill(12)
        img_path = os.path.join(self.img_folder, file_name + '.jpg')
        if self.is_train:
            points = np.array(a['unit']['keypoints']).reshape(self.num_class, 3).astype(np.float32)
        # gt_bbox = a['unit']['GT_bbox']
        gt_bbox = a['bbox']
        # gt_bbox = np.zeros_like(gt_bbox_temp)
        #
        # gt_bbox[0] = gt_bbox_temp[2]
        # gt_bbox[1] = gt_bbox_temp[3]
        # gt_bbox[2] = gt_bbox_temp[0]
        # gt_bbox[3] = gt_bbox_temp[1]
