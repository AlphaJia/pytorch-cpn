...
def _predict(output_shape, num_class):
    layers = []
...
class globalNet(nn.Module):
    def __init__(self, channel_settings, output_shape, num_class):
        ...
        for i in range(len(channel_settings)):
            laterals.append(_lateral(channel_settings[i]))
            predict.append(_predict(output_shape, num_class))
            if i != len(channel_settings) - 1:
                upsamples.append(_upsample())
